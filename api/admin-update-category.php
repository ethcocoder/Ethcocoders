...
$categoryName = trim($data['grade_name'] ?? '');
$categoryDescription = trim($data['description'] ?? '');
...
try {
    ...
    $stmt = $pdo->prepare("SELECT id FROM grade_categories WHERE grade_name = ? AND id != ?");
    ...
    $stmt = $pdo->prepare("UPDATE grade_categories SET grade_name = ?, description = ? WHERE id = ?");
